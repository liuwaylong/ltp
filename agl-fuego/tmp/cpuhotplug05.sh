...
until [ $LOOP_COUNT -gt $HOTPLUG05_LOOPS ]; do
	...
	grep "$time" $TMP/log_$$ >&2
#	grep "$time" $TMP/log_$$ | awk '{print $3 $4 $5 $6 $7 $8 }
	# Verify that at least some of the CPUs are offline
	NUMBER_CPU_OFF=$(grep "$time" $TMP/log_$$ | awk '{print $3$4$5$6$7$8 }' \
		|grep -i "^0.000.000.000.000.000.00" | wc -l)
	...
	NEW_NUMBER_CPU_OFF=$(grep "$time" $TMP/log_$$|awk '{print $3$4$5$6$7$8}' \
		| grep -i "^0.000.000.000.000.000.00"| wc -l)
	NUMBER_CPU_OFF=$((NUMBER_CPU_OFF-1))
	echo "$NUMBER_CPU_OFF $NEW_NUMBER_CPU_OFF" >&2
